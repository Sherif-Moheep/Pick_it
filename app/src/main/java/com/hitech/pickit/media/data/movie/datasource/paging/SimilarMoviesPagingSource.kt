...
class SimilarMoviesPagingSource(context: Context, private val movieApi: MovieService, private val movieId: Int) :
    BasePagingSource<Movie>(context) {
    override suspend fun fetchItems(page: Int): Result<List<Movie>, NetworkError>{
        return safeApiCall {
            val response = movieApi.fetchSimilarMovies(movieId,page)
